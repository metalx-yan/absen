...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $date = date("Y-m-d");
        $cek_absen = Absen::where(['users_id' => $users_id, 'date' => $date])
                            ->get()
                            ->first();
        ...
        $data_absen = Absen::where('users_id', $users_id)
                        ->orderBy('date', 'desc')
                        ->paginate(20);
    ...
    public function absen(Request $request){
        ...
        if (isset($request->btnIn)) {
             // cek double data
            $cek_double = $absen->where(['date' => $date, 'users_id' => $users_id])
                                ->count();
            if ($cek_double > 0) {
        ...
        elseif (isset($request->btnOut)) {
            $absen->where(['date' => $date, 'users_id' => $users_id])
                ->update([
                    'time_out' => $time,
                    'note' => $note]);
            return redirect()->back();
